...
bridging_from_get_pgd:
	...
	case IOCTL_CHANGE_VIEW:
			...
			pgd_entry = (void**)pgd_addr[arg];
                        ...
                        pml4=0;
                        involved_pml4=512;
                        //void ** pgd_original = (void **) current->mm->pgd;
                        /**/
			...
			for (i=0; i<involved_pml4; i++){ 
				...
				//pgd_entry[pml4] = temp;
                                ...
                                my_pdp = (void**)pgd_entry[pml4];
                                
                                //printk("pdp n° %d\n",i);
                                void * pd_address;
                                for(j=0; j<512; j++) {
                                 my_pdp[j]=(void *) __get_free_pages(GFP_KERNEL,0);
                                 //printk("pd n° %d di %d\n",j,i);
                                }
                                /**/
				pml4++;
...
bridging_from_pgd_release:/*da qui in poi esegue tutto fino al primo break*/
	...
	case IOCTL_RESTORE_VIEW:
			...
			for (i=0; i<involved_pml4; i++){            
                                my_pdp = (void **) pgd_entry[pml4];
                                //printk("pgd_entry[%d]=%p=my_pdp=%p\n",pml4,pgd_entry[pml4],my_pdp);
                                for(j=0; j<512; j++) {
                                 //printk("try to release pd number %d di pdp number %d\n",j,pml4);
                                 free_pages((unsigned long)my_pdp[j],0);
                                 //printk("release pd number %d di pdp number %d\n",j,pml4);
                                }
                                free_pages(pgd_entry[pml4],0);
                                //printk("release pdp number %d\n",pml4);
