 ...
 * 4-PAGE_MULTI_ACCESS: tracciamento degli accessi (contatore) a pagine di 4k da parte di 64 thread*/
	...
	typedef struct __header{
 	...
 	#define PAGE_MULTI_ACCESS
		...
		#define print_body(a_node) \
			do{\
				while(a_node!=NULL) {\
					unsigned long track = ((a_node->b).thread_tracking);\
					printk("(a_node->h).key_zone=%d\n",(a_node->h).key_zone);\
					for(j=0; j<MAX_NUM_THREAD_PER_ENTRY; j++) {\
 		...
 		typedef struct __body{
			type_tracking thread_tracking[MAX_NUM_THREAD_PER_ENTRY];
		}body;
		...
		#define update_body(pgd_index,a_node,pte_number) \
			do {\
				if(pgd_index>=MAX_NUM_THREAD_PER_ENTRY) {\
					...
					if((((a_node->b).thread_tracking[pgd_index])) == 0) {\
						(a_node->b).thread_tracking[pgd_index]--;\
		...
		#define print_body(a_node) \
			do{\
				while(a_node!=NULL) {\
					printk("(a_node->h).key_zone=%d\n",(a_node->h).key_zone);\
					for(j=0; j<MAX_NUM_THREAD_PER_ENTRY; j++) {\
						type_tracking ctb = (a_node->b).thread_tracking[j];\
						if(ctb)\
							printk("thread %d = %d\n",j,ctb);\
					}\
					a_node = (node *) ((a_node->h).next_node);\
		...
		#define update_body(pgd_index,a_node,pte_number) \
			do{\
				...
				page_node * a_page_node = (a_node->b).pages_nodes;\
				...
				if((a_page_node==NULL) || (pte_number < (a_page_node->pte))) {\
					...
					(a_page_node->next_page_node)= ((a_node->b).pages_nodes);\
					((a_node->b).pages_nodes) = (void *) a_page_node;\
					finalize_update_node:	address_tracking = &(a_page_node->thread_tracking);\
					set_bit((unsigned long)pgd_index,address_tracking);\
					goto end_add_node;\
				}\
		...
		#define print_body(a_node) \
			do{\
				while(a_node!=NULL) {\
					printk("(a_node->h).key_zone=%d\n",(a_node->h).key_zone);\
					page_node * pt = (page_node *) (a_node->b).pages_nodes;\
					while(pt!=NULL) {\
						printk("(pt->pte)=%d\n",pt->pte);\
						unsigned long track = (pt->thread_tracking);\
						for(j=0; j<MAX_NUM_THREAD_PER_ENTRY; j++) {\
							unsigned int ctb = CHECK_NODE(j,track);\
							if(ctb)\
								printk("thread %d = %d\n",j,ctb);\
		...
		#define update_body(pgd_index,a_node,pte_number) \
			do{\
				...
				if((a_page_node==NULL) || (pte_number < (a_page_node->pte))) {\
					...
					(a_page_node->thread_tracking)[pgd_index]++;\
					if(((a_page_node->thread_tracking)[pgd_index])==0) {\
						(a_page_node->thread_tracking)[pgd_index]--;\
						printk("non trackable access\n");\
					}\
					goto end_add_node;\
			...
			do{\
				while(a_node!=NULL) {\
					...
					while(pt!=NULL) {\
						printk("(pt->pte)=%d\n",pt->pte);\
						for(j=0; j<MAX_NUM_THREAD_PER_ENTRY; j++) {\
							type_tracking ctb = (pt->thread_tracking)[j];\
							if(ctb)\
								printk("thread %d = %d\n",j,ctb);\
						}\
						pt=(page_node *) (pt->next_page_node);\
	...
	#define SIZE_NODE (sizeof(node))
 	...
 	void update_hashbucket(void *,unsigned char);
 	void audit_hashbucket(void);
