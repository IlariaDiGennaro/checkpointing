...
//#include <sys/time.h>//cost analysis
...
/*ILARIA*/
void my_thread(char * cell){
	
	int pgd_ds; void * address;
	
	printf("my_thread: setting ancestor pgd\n");
	fflush(stdout);
	ioctl(fd,IOCTL_SET_ANCESTOR_PGD);
	
    printf("my_thread: getting pgd\n");
    fflush(stdout);
	pgd_ds = ioctl(fd, IOCTL_GET_PGD);  //ioctl call
	*cell = pgd_ds;
	printf("my thread: pgd=thread descriptor is %d\n",pgd_ds);
	fflush(stdout);
	
	/*printf("my thread: dummy schedule on\n");
	fflush(stdout);
	ioctl(fd,IOCTL_DUMMY_READ,pgd_ds); /*dummy schedule on*/
	
	printf("my thread: installing pgd\n");
	fflush(stdout);
	ioctl(fd,IOCTL_INSTALL_PGD,pgd_ds);
	
	ioctl(fd,IOCTL_ITERATE_PAGE,pgd_ds);
	
	/*printf("malloc-ing\n");
	address = malloc(4096);
	printf("address: %p\n",address);
	
	printf("my thread: sleeping for 5 seconds\n");
	fflush(stdout);
	sleep(5);
	printf("my thread: re-waking up\n");
	fflush(stdout);
	
	printf("my thread: uninstalling pgd\n");
	fflush(stdout);
	ioctl(fd,IOCTL_UNINSTALL_PGD,pgd_ds);
	
	printf("my thread: releasing pgd\n");
	fflush(stdout);
	ioctl(fd, IOCTL_RELEASE_PGD, pgd_ds);*/  //ioctl call 
...
int main(int argc, char**argv) {
	...
	unsigned char pgds [NUM_THREADS];
	...
	if (argc == 2 && strcmp(argv[1],"pgdget") == 0){
		...
		//ioctl(fd,IOCTL_GET_INFO_PGD,pgd_ds);
	//pgd_ds = ioctl(fd, IOCTL_INIT_PGD);  //ioctl call 
			...
			printf("pgd descriptor is %d\n",pgd_ds);
                        //
                        //ioctl(fd,IOCTL_DUMMY_READ,pgd_ds);
                        //ioctl(fd,IOCTL_REGISTER_THREAD,pgd_ds);	
                        ioctl(fd,IOCTL_INSTALL_PGD,pgd_ds);
                        /*malloc di prova per vedere page fault*/
                        //while(1){
                        //long memory = (long)malloc(4096);
                        //printf("malloc=%p,size=%d\n",(long)memory,sizeof(memory));
                        //memcpy(memory,"ciao ciao ciao",sizeof("ciao ciao ciao"));
                        //free(memory);
                        //sleep(3);
                        //}
                        //ioctl(fd,IOCTL_GET_INFO_VMAREA);
                        //ioctl(fd,IOCTL_GET_INFO_PGD,pgd_ds);
                        //for(i=0;i<128;i++) k+=i;
                        //printf("k=%d\n");
                        
                        //void * debug_mmap = mmap(0xffff87fff8000000,4096,PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS|MAP_POPULATE,0,0);
                        //void * debug_mmap = mmap(0x00000ffff8000000,4096,PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS|MAP_POPULATE,0,0);
                        //void * debug_mmap = mmap(0x0000000000001000,4096,PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS|MAP_POPULATE,0,0);
                        //void * debug_mmap = mmap(0x0000000000020000,4096,PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS|MAP_POPULATE,0,0);
                        //void * debug_mmap = mmap(0x0000000007E00000,4096,PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS|MAP_POPULATE,0,0);
                        void * debug_mmap = mmap(0x00000000001FF000,4096,PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS|MAP_POPULATE,0,0);
                        printf("debug_mmap=%p\n",debug_mmap);
                        printf("MAP_FAILED=%p\n",MAP_FAILED);
                        memcpy(debug_mmap,"ilaria",sizeof("ilaria"));
                        
                        
                        ioctl(fd,IOCTL_ITERATE_PAGE,pgd_ds);
                        
                        unsigned int k = 0;
                        while(k<10000000000) k++;
                        
                        gettimeofday(&tv2,NULL);
                        
                        double effective_time = (tv2.tv_sec -tv1.tv_sec) + ((tv2.tv_usec -tv1.tv_usec)/1000000.0);
...
*/
	...
	if(argc == 2 && strcmp(argv[1],"thread") == 0){
		for(i=0; i<NUM_THREADS; i++) {
	  		ret = pthread_create(&tid_id[i],NULL,my_thread,&(pgds[i]));
		...
		sleep(NUM_THREADS);
		for(i=0;i<NUM_THREADS;i++) {
			printf("releasing pgds[%d]=%d\n",i,pgds[i]);
			ioctl(fd,IOCTL_GET_INFO_PGD,pgds[i]);
			ioctl(fd,IOCTL_UNINSTALL_PGD,pgds[i]);
			ioctl(fd, IOCTL_RELEASE_PGD, pgds[i]);
		}
	...
	if(argc == 2 && strcmp(argv[1],"info") == 0){
		//ioctl(fd,IOCTL_UNINSTALL_PGD,0);
	  	ioctl(fd,IOCTL_AUDIT_TRACKING_STRUCTURE,0);
	  	//ioctl(fd,IOCTL_INSTALL_PGD,0);
	}
	
	if(argc == 2 && strcmp(argv[1],"prova") == 0) {
		//unsigned char char1 = (unsigned char)0xff;
	...
	if(argc==2 && strcmp(argv[1],"worstcase")==0) {
		ioctl(fd,IOCTL_DEBUG_WORST_CASE,0);	
	}
	
	if(argc==2 && strcmp(argv[1],"cleanstructure")==0) {
		ioctl(fd,IOCTL_CLEAN_ACCESSES,0);
	}
	/**/
